import type { ScreenCaptureEventMap } from "./events";
import { getLogger } from "../logger";

type EventName<EventMap> = Extract<keyof EventMap, string>;

const logger = getLogger("screen-capture-event-bus");

export interface ScreenCaptureEventBus {
  on<K extends EventName<ScreenCaptureEventMap>>(
    eventName: K,
    handler: (payload: ScreenCaptureEventMap[K]) => void | Promise<void>
  ): () => void;

  once<K extends EventName<ScreenCaptureEventMap>>(
    eventName: K,
    handler: (payload: ScreenCaptureEventMap[K]) => void | Promise<void>
  ): () => void;

  off<K extends EventName<ScreenCaptureEventMap>>(
    eventName: K,
    handler: (payload: ScreenCaptureEventMap[K]) => void | Promise<void>
  ): void;

  emit<K extends EventName<ScreenCaptureEventMap>>(
    eventName: K,
    payload: ScreenCaptureEventMap[K]
  ): void;

  removeAllListeners<K extends EventName<ScreenCaptureEventMap>>(eventName?: K): void;
}
    ...
    for (const handler of [...set]) {
      try {
        ...
        void Promise.resolve(result).catch((error) => {
          logger.error(
            { error, eventName },
            "Unhandled async error (Promise rejection) thrown by screen capture event handler"
          );
        });
      } catch (error) {
        logger.error(
          { error, eventName },
          "Unhandled error thrown by screen capture event handler"
        );
      }
    }
  }
...
export const screenCaptureEventBus: ScreenCaptureEventBus =
  new TypedEventBus<ScreenCaptureEventMap>();
